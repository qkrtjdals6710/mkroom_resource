							...
							<div class="row">
								<div class="form-group board1">
									...
									<input name="branchLocation" type="text" placeholder="분점" style="text-align:center;width:80px;height:24px;font-size:14px;"
										value='<c:out value="${questDetail.branchLocation}"/>' readonly="readonly">
									&nbsp;
							...
							<div class="col-sm-12 board3">
								...
								<div class="panel-body">
										...
										<div class="form-group fileUploadResult">
											<ul>
												<c:set var="imagePath" value='${questImageDetail.uploadPath += "/s_"
																				 += questImageDetail.uuid += "_" 
																				 += questImageDetail.fileName}'/>
												<li data-repopath='C:/myupload/' data-uploadpath='${questImageDetail.uploadPath}'
													data-uuid='${questImageDetail.uuid}' data-filename='${questImageDetail.fileName}'
													data-filetype='I'>
													<img src='${contextPath += "/displayThumbnail?fileName=C:/myupload/" += imagePath}' 
														 alt='No Image' style='height:18px; width: 18px;'/>&nbsp;<c:out value="${questImageDetail.fileName}"/>
													<span data-filename='${imagePath}' data-filetype='I'>[삭제]</span>
												</li>
											</ul>
										</div>
								</div><%-- /.panel-body --%>
...
<script type="text/javascript">
	...
	<%-- 업로드 결과 표시 함수 --%>
	function showUploadedFiles(questImageFile) {
		
		if(!questImageFile || questImageFile.length == 0) {
			return ;	
		}
		
		var fileUploadResult = $(".fileUploadResult ul") ;
		var str = "" ;
		$(questImageFile).each(function(i, obj){
			if (obj.fileType == "F") {
...
// 			               +  "<input type='hidden' name='fileInfos[" + i + "].repoPath' value='" + objli.data("repopath") + "' > "
	...
	$("#btnToList").on("click", function(){
		...
		//내가 하고도 이해불가. 이렇게 짜면 수정 열고 바로 취소를 누르면 기존 파일이 업로드 파일에서 삭제 되는 게 아닌가? 다행히 삭제가 안 되네. 이유를 모르겠네. 
		//기존 파일이 아니면 삭제되고. 
		var questForm = $("#questForm") ; 
		...
		if(targetFileName == null || targetFileName == "" || targetFileType == null || targetFileType == "" ){
			location.href = "${contextPath}/quest/detail?questName=" + "${questDetail.questName}" + "&branchLocation=" + "${questDetail.branchLocation}";
		}
		...
		$.ajax({
			...
			success: function(result) {
				questForm.attr("action", "${contextPath}/quest/detail?questName=" + "${questDetail.questName}" + "&branchLocation=" + "${questDetail.branchLocation}") ;
				questForm.attr("method", "get") ;
				questForm.submit() ;
