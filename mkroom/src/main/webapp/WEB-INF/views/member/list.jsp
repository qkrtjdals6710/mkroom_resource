...
<head>
  ...
  <body>
				...
				<div class="card">
					<div class="card-body">
						...
						<div style="text-align: right;">
							<input type="text" name="questPoint" id="questPoint" placeholder="추가할 point 값 입력" >
						</div>
						<table class="table table-striped">
							<thead>
								<tr>
									...
									<th scope="col">이메일</th>
									<th scope="col">할인 쿠폰</th>
									<th scope="col" colspan="2">퀘스트 포인트</th>
								</tr>
							</thead>
							...
							<c:forEach var="member" varStatus="memberStatus" items="${memberList}">
								<tbody id="memberTbody">	
									<tr>
										...
										<td class="pointUp">
											<button class="btn btn-danger">pointUp</button>
										</td>
									</tr>
								</tbody>	
							</c:forEach>
	...
	<script>
		...
		$("#memberTbody tr td").hover(function(e) {
			$(this).css({ "opacity" : 0.5 })
				   .click(function() {
					   
						var userId = $(this).parent("tr").children(".userId").text();
						var questPoint = $("#questPoint").val();
                		$("#questPoint").val("");
						   if($(this).attr("class") == "pointUp"){

		                      $.ajax({
		                        url: "${pageContext.request.contextPath}/admin/pointUp?questPoint=" + questPoint + "&userId=" + userId,
		                        method: "GET",
		                        success: function() {
		                        		questPoint = "";
		                        }
