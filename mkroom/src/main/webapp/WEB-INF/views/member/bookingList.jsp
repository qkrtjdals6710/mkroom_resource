  ...
  <body>
		...
		<ul class="sidebar-nav" id="sidebar-nav">
	      ...
	      <li class="nav-item">
	        ...
	        <ul id="forms-nav" class="nav-content collapse show" data-bs-parent="#sidebar-nav">
	          <li>
	            <a href="${pageContext.request.contextPath}/member/bookingList?userId=${param.userId}&pageNum=1&rowAmountPerPage=10" class="active">
	...
	<section class="section">
						...
						<table class="table table-striped">
							<thead>
								<tr>
									...
									<th scope="col">인원수</th>
									<th scope="col">결제 UID</th>
								</tr>
							</thead>
							<tbody id="memberBookingTbody">
								<c:forEach var="memberBooking" varStatus="memberBookingStatus" items="${memberBookingList}">
										<tr>
											<th scope="row">
												<c:out value="${memberBookingStatus.index+1}" />
											</th>
											<td class="userId">
												<c:out value="${memberBooking.userId}" />
											</td>
											<td class="userName">
												<c:out value="${memberBooking.userName}" />
											</td>
											<td class="questName">
												<c:out value="${memberBooking.questName}" />
											</td>
											<td class="branchLocation">
												<c:out value="${memberBooking.branchLocation}" />
											</td>
											<td class="selectTime">
												<c:out value="${memberBooking.selectTime}" />
											</td>
											<td class="teamMember">
												<c:out value="${memberBooking.teamMember}" />
											</td>
											<td class="impUid">
												<c:out value="${memberBooking.impUid}" />
											</td>
										</tr>
		...
		<nav aria-label="Page navigation example">
		  <ul class="pagination" style="justify-content:center;">
			...
			<c:if test="${pagingCreator.prev }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/bookingList?userId=${param.userId}&pageNum=${pagingCreator.startPagingNum - 1}&rowAmountPerPage=${param.rowAmountPerPage}">Previous</a>
				</li>
			</c:if>
			
			<c:forEach var="pageNum" begin="${pagingCreator.startPagingNum }" end="${pagingCreator.endPagingNum }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/bookingList?userId=${param.userId}&pageNum=${pageNum}&rowAmountPerPage=${param.rowAmountPerPage}">${pageNum}</a>
				</li>
			</c:forEach>
			
			<c:if test="${pagingCreator.next }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/bookingList?userId=${param.userId}&pageNum=${pagingCreator.endPagingNum + 1}&rowAmountPerPage=${param.rowAmountPerPage}">Next</a>
				</li>
			</c:if>
			
			<c:if test="${pagingCreator.next }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/bookingList?userId=${param.userId}&pageNum=${pagingCreator.lastPageNum}&rowAmountPerPage=${param.rowAmountPerPage}">End</a>
				</li>
			</c:if>
		  </ul>
		</nav>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$("#memberBookingTbody tr").hover(function() {
			...
			$("#rowAmountPerPage").change(function() {
				window.location.href = "${pageContext.request.contextPath}/member/bookingList?userId=${param.userId}&pageNum=1&rowAmountPerPage=" + $(this).val();
			})
		});
