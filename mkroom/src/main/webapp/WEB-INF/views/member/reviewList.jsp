  ...
  <body>
		...
		<ul class="sidebar-nav" id="sidebar-nav">
	      ...
	      <li class="nav-item"> 
	        ...
	        <ul id="components-nav" class="nav-content collapse show" data-bs-parent="#sidebar-nav">
	          <li>
	            <a href="${pageContext.request.contextPath}/member/reviewList?userId=${param.userId}&pageNum=1&rowAmountPerPage=10" class="active">
	...
	<section class="section">
			...
			<div class="col-lg-12">
				<div class="card">
					<div class="card-body">
						...
						<nav>
							<ol class="breadcrumb">
								<li class="breadcrumb-item active"><a href="${pageContext.request.contextPath}/member/reviewList?userId=${param.userId}&pageNum=1&rowAmountPerPage=10">리뷰글</a></li>
						...
						<table class="table table-striped">
							...
							<tbody id="memberReviewTbody">
								<c:forEach var="memberReview" varStatus="memberReviewStatus" items="${memberReviewList}">
	
									<tr>
										<th scope="row">
											<c:out value="${memberReviewStatus.index+1}" />
										</th>
										<td class="userId">
											<c:out value="${memberReview.userId}" />
										</td>
										<td class="userName">
											<c:out value="${memberReview.userName}" />
										</td>
										<td class="rno">
											<c:out value="${memberReview.rno}" />
										</td>
										<td class="questName">
											<c:out value="${memberReview.questName}" />
										</td>
										<td class="branchLocation">
											<c:out value="${memberReview.branchLocation}" />
										</td>
										<td class="rcontent">
											<c:out value="${memberReview.rcontent}" />
										</td>
										<td class="ratingStar">
											<c:out value="${memberReview.ratingStar}" />
										</td>
										<td class="rregDate">
											<c:out value="${memberReview.rregDate}" />
										</td>
										<td class="rmodDate">
											<c:out value="${memberReview.rmodDate}" />
										</td>
									</tr>
		...
		<nav aria-label="Page navigation example">
		  <ul class="pagination" style="justify-content:center;">
			...
			<c:if test="${pagingCreator.prev }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/reviewList?userId=${param.userId}&pageNum=${pagingCreator.startPagingNum - 1}&rowAmountPerPage=${param.rowAmountPerPage}">Previous</a>
				</li>
			</c:if>
			
			<c:forEach var="pageNum" begin="${pagingCreator.startPagingNum }" end="${pagingCreator.endPagingNum }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/reviewList?userId=${param.userId}&pageNum=${pageNum}&rowAmountPerPage=${param.rowAmountPerPage}">${pageNum}</a>
				</li>
			</c:forEach>
			
			<c:if test="${pagingCreator.next }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/reviewList?userId=${param.userId}&pageNum=${pagingCreator.endPagingNum + 1}&rowAmountPerPage=${param.rowAmountPerPage}">Next</a>
				</li>
			</c:if>
			
			<c:if test="${pagingCreator.next }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/reviewList?userId=${param.userId}&pageNum=${pagingCreator.lastPageNum}&rowAmountPerPage=${param.rowAmountPerPage}">End</a>
				</li>
			</c:if>
		  </ul>
		</nav>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$("#memberReviewTbody tr").hover(function() {
				$(this).css({ "opacity" : 0.5 })
					   .click(function() {
							window.location.href = "${pageContext.request.contextPath}/quest/detail?questName=" + $(this).children("td.questName").text().trim()
								 + "&branchLocation=" + $(this).children("td.branchLocation").text().trim();
					   })	
			}, function() {
				$(this).css({ "opacity" : 1  });
			});
			...
			$("#rowAmountPerPage").change(function() {
				window.location.href = "${pageContext.request.contextPath}/member/reviewList?userId=${param.userId}&pageNum=1&rowAmountPerPage=" + $(this).val();
			})
		});
