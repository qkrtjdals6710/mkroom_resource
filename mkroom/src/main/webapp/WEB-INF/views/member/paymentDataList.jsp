  ...
  <body>
		...
		<ul class="sidebar-nav" id="sidebar-nav">
	      ...
	      <li class="nav-item">
	        ...
	        <ul id="forms-nav" class="nav-content collapse show" data-bs-parent="#sidebar-nav">
	          ...
	          <li>
	            <a href="${pageContext.request.contextPath}/member/paymentDataList?userId=${param.userId}&pageNum=1&rowAmountPerPage=10" class="active">
	...
	<section class="section">
				...
				<div class="card">
					<div class="card-body">
						...
						<table class="table table-striped">
							...
							<tbody id="memberPaymentDataTbody">
								<c:forEach var="memberPaymentData" varStatus="memberPaymentDataStatus" items="${memberPaymentDataList}">
										<tr>
											<th scope="row">
												<c:out value="${memberPaymentDataStatus.index+1}" />
											</th>
											<td class="userId">
												<c:out value="${memberPaymentData.userId}" />
											</td>
											<td class="userName">
												<c:out value="${memberPaymentData.userName}" />
											</td>
											<td class="impUid">
												<c:out value="${memberPaymentData.impUid}" />
											</td>
											<td class="productname">
												<c:out value="${memberPaymentData.productName}" />
											</td>
											<td class="branchlocation">
												<c:out value="${memberPaymentData.branchLocation}" />
											</td>
											<td class="selecttime">
												<c:out value="${memberPaymentData.selectTime}" />
											</td>
											<td class="amount">
												<c:out value="${memberPaymentData.amount}" />
											</td>
											<td class="paidat">
												<c:out value="${memberPaymentData.paidAt}" />
											</td>
										</tr>
		...
		<nav aria-label="Page navigation example">
		  <ul class="pagination" style="justify-content:center;">
			...
			<c:if test="${pagingCreator.prev }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/paymentDataList?userId=${param.userId}&pageNum=${pagingCreator.startPagingNum - 1}&rowAmountPerPage=${param.rowAmountPerPage}">Previous</a>
				</li>
			</c:if>
			
			<c:forEach var="pageNum" begin="${pagingCreator.startPagingNum }" end="${pagingCreator.endPagingNum }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/paymentDataList?userId=${param.userId}&pageNum=${pageNum}&rowAmountPerPage=${param.rowAmountPerPage}">${pageNum}</a>
				</li>
			</c:forEach>
			
			<c:if test="${pagingCreator.next }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/paymentDataList?userId=${param.userId}&pageNum=${pagingCreator.endPagingNum + 1}&rowAmountPerPage=${param.rowAmountPerPage}">Next</a>
				</li>
			</c:if>
			
			<c:if test="${pagingCreator.next }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/paymentDataList?userId=${param.userId}&pageNum=${pagingCreator.lastPageNum}&rowAmountPerPage=${param.rowAmountPerPage}">End</a>
				</li>
			</c:if>
		  </ul>
		</nav>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$("#memberPaymentDataTbody tr").hover(function() {
				$(this).css({ "opacity" : 0.5 })
					   .click(function() {
							window.location.href = "${pageContext.request.contextPath}/member/bookingAndPaymentDataDetail?userId=" + $(this).children("td.userId").text().trim() + "&impUid=" + $(this).children("td.impUid").text().trim();
					   })	
			}, function() {
				$(this).css({ "opacity" : 1  });
			});
			...
			$("#rowAmountPerPage").change(function() {
				window.location.href = "${pageContext.request.contextPath}/member/paymentDataList?userId=${param.userId}&pageNum=1&rowAmountPerPage=" + $(this).val();
			})
		});
