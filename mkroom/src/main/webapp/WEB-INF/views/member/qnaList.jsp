  ...
  <body>
		...
		<ul class="sidebar-nav" id="sidebar-nav">
	      ...
	      <li class="nav-item"> 
	        ...
	        <ul id="components-nav" class="nav-content collapse show" data-bs-parent="#sidebar-nav">
	          ...
	          <li>
	            <a href="${pageContext.request.contextPath}/member/qnaList?userId=${param.userId}&pageNum=1&rowAmountPerPage=10" class="active">
	...
	<section class="section">
				...
				<div class="card">
					<div class="card-body">
						...
						<table class="table">
							...
							<tbody id="memberQnaTbody">
								<c:forEach var="memberQna" varStatus="memberQnaStatus" items="${memberQnaList}">
	
									
										<c:if test="${memberQna.breplycnt != 0}">
											<tr class="table-Secondary">
												<th scope="row">
													<c:out value="${memberQnaStatus.index+1}" />
												</th>
												<td class="userId">
													<c:out value="${memberQna.userId}" />
												</td>
												<td class="userName">
													<c:out value="${memberQna.userName}" />
												</td>
												<td class="bno">
													<c:out value="${memberQna.bno}" />
												</td>
												<td class="btitle">
													<c:out value="${memberQna.btitle}" />
												</td>
												<td class="bcontent">
													<c:out value="${memberQna.bcontent}" />
												</td>
												<td class="breplycnt">
		...
		<nav aria-label="Page navigation example">
		  <ul class="pagination" style="justify-content:center;">
			...
			<c:if test="${pagingCreator.prev }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/qnaList?userId=${param.userId}&pageNum=${pagingCreator.startPagingNum - 1}&rowAmountPerPage=${param.rowAmountPerPage}">Previous</a>
				</li>
			</c:if>
			
			<c:forEach var="pageNum" begin="${pagingCreator.startPagingNum }" end="${pagingCreator.endPagingNum }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/qnaList?userId=${param.userId}&pageNum=${pageNum}&rowAmountPerPage=${param.rowAmountPerPage}">${pageNum}</a>
				</li>
			</c:forEach>
			
			<c:if test="${pagingCreator.next }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/qnaList?userId=${param.userId}&pageNum=${pagingCreator.endPagingNum + 1}&rowAmountPerPage=${param.rowAmountPerPage}">Next</a>
				</li>
			</c:if>
			
			<c:if test="${pagingCreator.next }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/member/qnaList?userId=${param.userId}&pageNum=${pagingCreator.lastPageNum}&rowAmountPerPage=${param.rowAmountPerPage}">End</a>
				</li>
			</c:if>
		  </ul>
		</nav>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$("#memberQnaTbody tr").hover(function() {
				$(this).css({ "opacity" : 0.5 })
					   .click(function() {
							window.location.href = "${pageContext.request.contextPath}/qna/qnadetail?bno=" + $(this).children("td.bno").text().trim();
					   })	
			}, function() {
				$(this).css({ "opacity" : 1  });
			});
			...
			$("#rowAmountPerPage").change(function() {
				window.location.href = "${pageContext.request.contextPath}/member/qnaList?userId=${param.userId}&pageNum=1&rowAmountPerPage=" + $(this).val();
			})
		});
