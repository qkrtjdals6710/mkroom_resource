	...
	<aside id="sidebar" class="sidebar">
		...
		<ul class="sidebar-nav" id="sidebar-nav">
	      ...
	      <security:authorize access="hasRole('ADMIN') or hasRole('BMANAGER')">
	        ...
	        <security:authorize access="hasRole('ADMIN')">
		        <li class="nav-item">
		          <a class="nav-link" data-bs-target="#delete-nav" data-bs-toggle="collapse" href="#">
		            <i class="bi bi-dash-circle"></i>
		            <span>삭제요청</span>
		            <i class="bi bi-chevron-down ms-auto"></i>
		          </a>
		          
		          <ul id="delete-nav" class="nav-content collapse show" data-bs-parent="#sidebar-nav">
		            <li>
		            ...
		            <li>
		              <a href="${pageContext.request.contextPath}/admin/deleteEvent" class="active">
				...
				<div class="card">
					<div class="card-body">
						...
						<table class="table table-striped">
							...
							<c:forEach var="delEvent" varStatus="delEventStatus" items="${delEventList}">
								<tbody id="delEventTbody">	
									<tr>
										<th scope="row">
											<c:out value="${delEventStatus.index+1}" />
										</th>
										<td class="btitle">
											<c:out value="${delEvent.btitle}" />
										</td>
										<td class="bwriter">
											<c:out value="${delEvent.bwriter}" />
										</td>
										<td class="bno">
											<c:out value="${delEvent.bno}" />
										</td>
										<td class="bregDate">
											<c:out value="${delEvent.bregDate}" />
										</td>
										<td class="bmodDate">
											<c:out value="${delEvent.bmodDate}" />
										</td>
										<td class="bviewCnt">
											<c:out value="${delEvent.bviewCnt}" />
										</td>
										<td class="delete">
		...
		<nav aria-label="Page navigation example">
		  <ul class="pagination" style="justify-content:center;">
		    <c:if test="${pagingCreator.prev }">
				<li class="page-item">
					<a class="page-link" href="?userId=${principal.username}&pageNum=1&rowAmountPerPage=${param.rowAmountPerPage}">First</a>
				</li>
			</c:if>
			<c:if test="${pagingCreator.prev }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/admin/deleteEvent?userId=${principal.username}&pageNum=${pagingCreator.startPagingNum - 1}&rowAmountPerPage=${param.rowAmountPerPage}">Previous</a>
				</li>
			</c:if>
			
			<c:forEach var="pageNum" begin="${pagingCreator.startPagingNum }" end="${pagingCreator.endPagingNum }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/admin/deleteEvent?userId=${principal.username}&pageNum=${pageNum}&rowAmountPerPage=${param.rowAmountPerPage}">${pageNum}</a>
				</li>
			</c:forEach>
			
			<c:if test="${pagingCreator.next }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/admin/deleteEvent?userId=${principal.username}&pageNum=${pagingCreator.endPagingNum + 1}&rowAmountPerPage=${param.rowAmountPerPage}">Next</a>
				</li>
			</c:if>
			
			<c:if test="${pagingCreator.next }">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/admin/deleteEvent?userId=${principal.username}&pageNum=${pagingCreator.lastPageNum}&rowAmountPerPage=${param.rowAmountPerPage}">End</a>
				</li>
			</c:if>
		  </ul>
		</nav>
...
<security:authorize access="isAuthenticated()">
	...
	<script>
		...
		$(document).ready(function() {
			...
			$("#delEventTbody tr td").hover(function() {
				$(this).css({ "opacity" : 0.5 })
					   .click(function() {
						   if($(this).attr("class") == "delete"){
							   window.location.href = "${pageContext.request.contextPath}/admin/deleteDetailEvent?bno=" + $(this).parent("tr").children("td.bno").text().trim();
						   } else{
							   window.location.href = "${pageContext.request.contextPath}/eventboard/detail?bno=" + $(this).parent("tr").children("td.bno").text().trim();
						   }
					   })	
			}, function() {
				$(this).css({ "opacity" : 1  });
			...
			$("#rowAmountPerPage").change(function() {
				window.location.href = "${pageContext.request.contextPath}/admin/deleteEvent?userId=${principal.username}&pageNum=1&rowAmountPerPage=" + $(this).val();
			})
		});
